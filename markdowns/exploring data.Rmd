...
It may have been a minute since you took a math class, so let's take a step back and think about what that natural log (ln) is doing there. The [natural log (ln)](https://betterexplained.com/articles/demystifying-the-natural-logarithm-ln/) can be thought of the amount of time it takes something to grow exponentially from one. So ln(1) will be 0, since it takes no time to get to where you currently are. 

In other words, if a word appears in all three documents it will have a idf of 0. The more common the word across documents, the more it is discounted. This is in fact a good method to find context specific stop words in a collection of documents. Words that appear frequently in every document (and have a IDF score of 0) may be good candidates for stop words.  

### TF-IDF

Because you multiple the term frequency and the inverse document frequency togetehr, in practice, this means that the tf **boosts** common words within a document, and the idf **discounts** words that are common across documents.  
...
The only way to know sure would be to read the book, but this is of course more time efficient. 
