...
I think that `tm` really shines when performing data cleaning and data transformation tasks. The primary function for content transformations is acheived using `tm_map`. `tm_map` may work a little counterintuitively if you are well versed in R, especially the tidyverse. The first arguement is a corpus. The second argument is a function to be applied to that corpus. Any arguements after that are arguments to be applied to the function. Wait what? Let me explain.  
...
europe_clean = tm_map(europe, stripWhitespace)
...
You can also use the `content_transformer` function to apply non-tm functions to your corpus. For example, lets use `gsub` to replace all instances of "spain" with "the glorious kingdom of spain". We will first use the `tolower` function to make sure cases aren't a problem. 
...
A document term matrix or term document matrix is a way of representing words as a table. The most simple implementation of this will have words as columns, the documents as rows, and if there is a presence of that word in the document as a 1 or 0. In `tm`, the values represent the number of times a word appears in a document. Creating a document term matrix is quite easy in `tm`. 
...
Document term matrixes are not the most efficient way to store data. Often times they are quite sparse, meaning that most values in the matrix are 0. There are some interesting ways to represent sparse matrixes 